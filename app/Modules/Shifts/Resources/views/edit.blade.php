                            ...
                            <div class="modern-checkbox-item">
                                ...
                                <label class="custom-toggle">
                                    <input 
                                        ...
                                        type="checkbox" 
                                        id="team_{{ $team->id }}"
                                        {{ (old('teams') && in_array($team->id, old('teams'))) || (!old('teams') && in_array($team->id, $shift->teams->pluck('id')->toArray())) ? 'checked' : '' }}
                                    >
                        ...
                        <div class="modern-checkbox-item">
                            ...
                            <label class="custom-toggle custom-toggle-danger">
                                <input 
                                    ...
                                    type="checkbox" 
                                    id="day_{{ $key }}"
                                    {{ (old('days') && in_array($key, old('days'))) || (!old('days') && in_array($key, json_decode($shift->days))) ? 'checked' : '' }}
                                >
                                <span class="custom-toggle-slider rounded-circle" data-label-off="No" data-label-on="Yes"></span>
                            </label>
                        </div>
        ...
        <div class="col-md-6">
            <div class="modern-form-group">
                ...
                <input 
                    ...
                    id="time_from" 
                    name="time_from" 
                    value="{{ old('time_from', \Carbon\Carbon::parse($shift->time_from)->format('H:i')) }}"
                    required
                >
                @error('time_from')
                    <div class="modern-form-error">{{ $message }}</div>
        ...
        <div class="col-md-6">
            <div class="modern-form-group">
                ...
                <input 
                    ...
                    id="time_to" 
                    name="time_to" 
                    value="{{ old('time_to', \Carbon\Carbon::parse($shift->time_to)->format('H:i')) }}"
                    required
                >
                @error('time_to')
                    <div class="modern-form-error">{{ $message }}</div>
...
@include('partials.modern-form-wrapper', [
    ...
    'breadcrumbs' => [
        ...
        ['label' => 'Edit ' . $shift->name, 'url' => null]
    ],
    'headerButtons' => [
        [
            'label' => 'Back to List', 
    ...
    'formTitle' => 'Edit ' . $shift->name . '\'s data',
