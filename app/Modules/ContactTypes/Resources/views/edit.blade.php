        ...
        <div class="col-md-6">
            <div class="modern-form-group">
                ...
                <input 
                    ...
                    value="{{ old('name', $contactType->name) }}"
                    required
                >
                @error('name')
            ...
            <div class="modern-form-group">
                ...
                >
                    @foreach ($teams as $team)
                        <option value="{{ $team->id }}" {{ old('team_id', $contactType->team_id) == $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                    @endforeach
                </select>
                @error('team_id')
                    <div class="modern-form-error">{{ $message }}</div>
        ...
        <div class="col-md-12">
            <div class="modern-form-group">
                ...
                <textarea 
                    ...
                    rows="4"
                    required
                >{{ old('description', $contactType->description) }}</textarea>
                @error('description')
                    <div class="modern-form-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="modern-form-group">
                ...
                <div class="range-slider-wrapper range-slider-centered">
                    <input 
                        ...
                        min="1" 
                        max="1000" 
                        value="{{ old('sla', $contactType->sla) }}"
                        required
                    >
                    <span class="range-value-display" id="sla-display">{{ old('sla', $contactType->sla) }}</span>
                </div>
                @error('sla')
                    <div class="modern-form-error">{{ $message }}</div>
                @enderror
...
@include('partials.modern-form-wrapper', [
    ...
    'breadcrumbs' => [
        ...
        ['label' => 'Edit ' . $contactType->name, 'url' => null]
    ],
    'headerButtons' => [
        [
            'label' => 'Back to List', 
    ...
    'formTitle' => 'Edit ' . $contactType->name . '\'s data',
